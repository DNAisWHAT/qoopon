...
try {
    ...
    await page.goto('https://www.qoo10.com/gmkt.inc/Event/RouletteQ.aspx?frame_id=i_RouletteQ');
    await page.evaluate(() => {
        document.querySelector('#td_today').childNodes[3].childNodes[0].onclick();
    })
    await page.waitForNavigation({ waitUntil: 'networkidle0' });

    await page.close();
    const newPage = await browser.newPage();
    await newPage.goto('https://www.qoo10.com/gmkt.inc/Event/RouletteQ.aspx?frame_id=i_RouletteQ');
    await newPage.evaluate(() => {
        document.querySelector('#roulette_board').contentWindow.document.querySelector('#btn_start').click();
    })
    await newPage.waitForNavigation({ waitUntil: 'networkidle0' });
    await new Promise((newPage) => setTimeout(newPage, 5000000));
    await page.goto('https://www.qoo10.com/gmkt.inc/MyCoupon/MyCouponList.aspx?global_order_type=L');
    await page.evaluate(() => {
        // 30프로 쿠폰 링크를 찾는 로직
        document.querySelector('.sbj').click();
        document.querySelector('#available_all_coupon_list').childNodes[1].querySelector('.ds').textContent;
        document.querySelector('#menu_A_2899108760089029938').childNodes[0].childNodes[1].childNodes[0].click();
    });
    await page.type('#to_email', 'aln1@catgroup.uk');
    await page.type('#txt_message', '.');
    await page.click('#send_gift_btn');


    // page.mouse.click(ticket_coordinates['x'], ticket_coordinates['y']);
    // await new Promise((page) => setTimeout(page, 1000));
    // await page.goto('https://www.qoo10.com/gmkt.inc/Event/qchance.aspx');
    // page.mouse.click(roulette_coordinates['x'], roulette_coordinates['y']);
  ...
  for (let link of songjangs) {
      ...
      const songjang_result = await page.evaluate(()=> {
          ...
          try {
              ...
              takbae_status = "배송 대기중 또는 배송 완료중...";
          }
  ...
  console.log(`배송 중인 모든 상품의 배송 정보 : ${post_logs}`);
...
/*       const shippingPages = pages.map(
    ...
    if (postLogs.toString() !== '')
    {
        fs.writeFile(`${combo[0]}_${dateString}.txt`, postLogs.toString(), { encoding: 'utf8' }, (err) => {
          ...
          console.log(`${combo[0]} : ALL done. closing...`);
          // await browser.close();
    }
